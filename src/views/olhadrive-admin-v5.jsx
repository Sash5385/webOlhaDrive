...
const TAG_PRESETS = [
  ...
  { id:"check",  icon:"🔍", label:"Перевірка", color: "#2dd4bf" },
...
@keyframes pulse-ring {
  0%,100% { box-shadow: 4px 4px 9px rgba(0,0,0,0.5), -3px -3px 8px rgba(255,255,255,0.28), 0 0 0 0 rgba(255,90,60,0.6); }
  50%     { box-shadow: 4px 4px 9px rgba(0,0,0,0.5), -3px -3px 8px rgba(255,255,255,0.28), 0 0 0 6px rgba(255,90,60,0); }
}
...
const computeBookingPrice = (b, servicesList) => {
  ...
  const basePrice = svc
    ...
    : b.price && b.durationHours
      ...
      : (b.price || 0);
  // Знижка учня (грн/год, фіксована сума) — діє на годину, масштабується на тривалість.
  const discount = (b.discount || 0) * (b.durMin / 60);
  return Math.max(0, Math.round(basePrice + (b.surcharge || 0) - discount));
};
...
function MonthCalendarSheet({ bookings, onClose, onPickDate }) {
  ...
  // will-change: будь-який з них перетворює предка на containing block
...
function ScheduleView({ settings, setSettings, onSlotClick, onEmptySlotClick, bookings, setBookings, activeDragIds, navTo, slotExistsRef, openSlotsRef, jumpTarget, setJumpTarget, onViewStudent }) {
  const { BG, BG_DEEP, SURFACE, SURF_HI, SURF_LO, BORDER, TEXT, DIM, FAINT, ACCENT, BLUE, GREEN, GOLD, RED, SO, STRIPE_A, STRIPE_B } = useContext(ThemeContext);
  ...
  const toggleDayBlocked = (dateStr) => {
    ...
    if (wasClosed) {
      ...
      // "відкриті" слоти, попри позначку "закрито".
  ...
  const [personalEventView, setPersonalEventView] = useState(null); // booking for viewing
  ...
  return (
            ...
            <div key={absDay} style={{
              ...
              <div
                ...
                style={{
                  ...
                  background: isLight ? `linear-gradient(135deg,${SURF_LO},${BG_DEEP})` : `linear-gradient(135deg,color-mix(in srgb,${SURF_HI} 50%,transparent),color-mix(in srgb,${SURF_LO} 50%,transparent))`,
                ...
                return (
                  <div key={`os-${time}`}
                    ...
                    style={{
                      ...
                      opacity: isBeingDragged || isBeingResized ? 0.95 : isSticky ? (isLight ? 0.9 : 0.9) : (isLight ? 0.85 : 0.85),
                      background: bg,
                      ...
                      // На минулих днях onPointerDown одразу виходить (isPastDay) — ні
                      // drag/resize, ні ручний скрол там не вмикаються, тож touch-action:none
                      // лишав дотик по порожньому слоту "мертвим": ні нативне panning, ні
                  ...
                  <div key={b.id} style={{
                    ...
                    <div
                      ...
                      {isPersonal && height >= 14 && (() => {
                        ...
                        return (
                          <div style={{position:"absolute",top:2,left:4,right:2,bottom:2,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:1,overflow:"hidden",textAlign:"center"}}>
                            <span style={{fontSize:Math.min(fs+1,11),lineHeight:1}}>📌</span>
                            {nameLines.slice(0,2).map((word,i)=>(
                              <div key={i} style={{fontSize:fs,fontWeight:700,color:"#2dd4bf",lineHeight:1.2,whiteSpace:"normal",textAlign:"center",wordBreak:"break-word",overflowWrap:"anywhere",textShadow:"none"}}>{word}</div>
                            ))}
                          </div>
                        );
                      })()}
                  ...
                  .reduce((s,b)=>{
                    ...
                    const basePrice = svc
                      ...
                      : b.price && b.durationHours
                        ? Math.round((b.price/(b.durationHours*60))*b.durMin)
                        : (b.price||0);
                    return s+basePrice+(b.surcharge||0);
                  },0);
                if (daySum<=0) return null;
                return (
                  <div style={{
                    // zIndex нижче за картки запису (zIndex:2, рядок ~2650) — щоб
      ...
      <div onClick={()=>setBubbleData(null)} style={{position:"fixed",inset:"0 0 80px 0",zIndex:100}}>
        <div onClick={e=>e.stopPropagation()} style={{
          ...
          <button onClick={()=>{
            ...
            setPersonalEventData({ dateStr: absDayToDateStr(bubbleData.day), time: `${sh}:${sn}` });
            setBubbleData(null);
          }} style={{
            width:"100%",padding:"9px 12px",borderRadius:10,border:"none",cursor:"pointer",
            background:"rgba(45,212,191,0.15)",
            color:"#2dd4bf",fontSize:12,fontWeight:800,marginTop:6,
    ...
    <NewBookingModal data={formData} onClose={()=>setFormData(null)}
      onConfirm={b=>{
        if (b.id) {
          ...
          // Клієнт без акаунту (гість) — пишемо в те саме дерево bookings/ під
          // синтетичним guest_<телефон> uid (не спільний "admin" і не окремий
          // bookings_by_phone). Інакше запис або мішається з іншими гостьовими
          // записами під тим самим uid, або взагалі невидимий для адмінки й не
          // отримує логіки з onBookingChanged (блокування слоту, синк з
          // Google Calendar, звільнення черги тощо).
          if (b.userId) {
    ...
    {personalEventView && (
      <div onClick={()=>setPersonalEventView(null)} style={{
        ...
        <div onClick={e=>e.stopPropagation()} style={{
          width:300,background:panel(SURF_HI, SURFACE),borderRadius:20,overflow:"hidden",
          boxShadow:`0 8px 40px ${shade(0.5)}, 0 0 0 1.5px rgba(45,212,191,0.3)`,
        }}>
          <div style={{padding:"14px 16px 12px",background:"rgba(45,212,191,0.1)",borderBottom:"1px solid rgba(45,212,191,0.15)"}}>
          ...
          <button onClick={deletePersonalEvent} style={{
...
function CreateSlotSheet({ data, settings, onClose }) {
  const { BG_DEEP, BORDER, TEXT, DIM, FAINT, GREEN } = useContext(ThemeContext);
  const { glow, shade } = useFX();
      ...
      <div onClick={closing ? undefined : _close} style={{
        ...
        <div onClick={e=>e.stopPropagation()}
          ...
          <div style={{padding:"10px 10px 4px",display:"flex",gap:8}}>
            ...
            {booking.status === "pending" && <button onClick={() => { onAction("confirm", booking); _close(); }} style={{
              flex:1,display:"flex",alignItems:"center",justifyContent:"center",gap:7,
              padding:"11px",borderRadius:14,border:"none",cursor:"pointer",fontFamily:"inherit",
              background:"rgba(158,224,122,0.15)",color:"#5fb83d",fontSize:13,fontWeight:800,
            }}>✓ Підтвердити</button>}
            <button onClick={() => onAction("chat", booking)} style={{
              flex:1,display:"flex",alignItems:"center",justifyContent:"center",gap:7,
              padding:"11px",borderRadius:14,border:"none",cursor:"pointer",fontFamily:"inherit",
              background:ink(0.1),color:BLUE,fontSize:13,fontWeight:800,
            }}>{IcoChat} Чат</button>
          </div>
          <div style={{padding:"0 10px 4px",display:"flex",gap:8}}>
...
function TemplatesView() {
  ...
  const inputSt = {
    ...
    background:`linear-gradient(135deg,${BG_DEEP},${SURFACE_LO})`,
    border:"none", outline:"none", color:TEXT, fontSize:13,
    padding:"10px 13px", borderRadius:12, boxShadow:SHADOW_IN,
      ...
      {TMETA.map(meta => {
        ...
        return (
          <Card key={meta.type} style={{padding:20}}>
            ...
            <div style={{marginBottom:10}}>
              <div style={{fontSize:10,color:TEXT_FAINT,fontWeight:700,letterSpacing:1.5,textTransform:"uppercase",marginBottom:5}}>Заголовок</div>
            ...
            <div style={{marginBottom:10}}>
              <div style={{fontSize:10,color:TEXT_FAINT,fontWeight:700,letterSpacing:1.5,textTransform:"uppercase",marginBottom:5}}>Текст</div>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12,alignItems:"center"}}>
              ...
              <span style={{fontSize:11,color:TEXT_FAINT}}>— вставити змінну</span>
            </div>

            <div style={{padding:"12px 14px",borderRadius:14,background:ink(0.04),border:`1px solid ${ink(0.06)}`,marginBottom:12}}>
              <div style={{fontSize:10,color:TEXT_FAINT,fontWeight:700,letterSpacing:1.5,textTransform:"uppercase",marginBottom:6}}>Превью</div>
              <div style={{fontSize:13,fontWeight:700,color:TEXT,marginBottom:2}}>{applyV(draft.title,meta.sample)}</div>
              <div style={{fontSize:12,color:TEXT_DIM,lineHeight:1.4}}>{applyV(draft.body,meta.sample)}</div>
            </div>
      ...
      <Card style={{padding:20}}>
        ...
        {log.length===0 ? (
          ...
          <div key={item.id} style={{
            ...
            <div style={{fontSize:11,color:TEXT_FAINT,flexShrink:0,minWidth:76,paddingTop:1}}>{fmtTs(item.sentAt)}</div>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:12,fontWeight:700,color:TEXT,marginBottom:2}}>{item.title}</div>
              <div style={{fontSize:11,color:TEXT_DIM,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{item.body}</div>
            </div>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{display:"flex",gap:6}}>
                  <select
                    ...
                    style={{flex:1,padding:"9px 4px",borderRadius:10,background:SURF_LO,border:`1px solid ${BORDER}`,color:TEXT,fontSize:13,outline:"none",boxSizing:"border-box"}}
                  >
                    {Array.from({length:24},(_,h)=>String(h).padStart(2,"0")).map(h=>(
                      <option key={h} value={h}>{h}</option>
                    ))}

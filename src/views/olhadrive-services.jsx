          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr auto",gap:10,alignItems:"center"}}>
            ...
            <Inset style={{padding:"8px 12px",display:"flex",alignItems:"center",gap:6}}>
              <input type="number" value={form.nextPrice ?? ""} min={0} placeholder="ціна"
                ...
                style={{flex:1,background:"transparent",border:"none",outline:"none",color:GOLD,fontSize:15,fontWeight:800,width:70}}/>
...
`;
  ...
  useEffect(() => {
    get(ref(db, 'admin_data/services')).then(snap => {
      const d = snap.val();
      if (!dirtyRef.current && Array.isArray(d)) setServices(d);
      setLoaded(true);

...
// (webOlhaDriveClient/src/pages/cabinet/ProfileTab.jsx), тут лише для показу адміну.
...
function StudentDetailSheet({ s, onClose, onUpdate, onDelete, onBlock, autoOpenHistory }) {
  ...
  // Cloud Function onNewStudentRegistered (webOlhaDrive/functions/index.js)
  ...
  const generateInvite = async () => {
    ...
    try {
      ...
      setInviteLink(`https://olhadrive.kiev.ua/auth?invite=${inviteRef.key}`);
      setInviteOpen(true);
    } catch (e) {
...
`;
  ...
  // Останній запис і кількість уроків на учня — з уже завантажених bookings
  // (той самий стрім, що й розклад), без окремого запиту на кожного учня.
  ...
  const list = students
    ...
    .sort((a,b)=>{
      if (sortMode === "lastBooking") {
        ...
        return kb.localeCompare(ka); // найновіші спочатку
      }
      if (sortMode === "createdAt") return (b.createdAt||0) - (a.createdAt||0); // найновіші спочатку
      if (sortMode === "lessons") {
        const la = studentStats[a.id]?.lessons || 0;
        const lb = studentStats[b.id]?.lessons || 0;
        return lb - la; // більше уроків спочатку
      }
      return (a.name||"").localeCompare(b.name||""); // алфавіт
    });

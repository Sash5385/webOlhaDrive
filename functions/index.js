  ...
  for (const [deviceId, token] of devices) {
    try {
      ...
      if (e.code === "messaging/registration-token-not-registered" ||
        ...
        // індекс для broadcast-розсилок, і якщо його не чистити тут, студент
        // назавжди лишається у списку розсилки, хоча реальний токен вже видалено.
    ...
    for (const [id, ev] of Object.entries(snap.val())) {
      ...
      if (diffMs <= thresholdMs) {
        ...
        await pushAdmin(
          ...
          { url: `https://admin.olhadrive.kiev.ua/?date=${ev.date}`, alarm: "1" }
        );
...
exports.onPushTask = onValueCreated(
  ...
  async (event) => {
    ...
    const url = `https://olhadrive.kiev.ua/cabinet?date=${date}${slotsArr[0] ? `&time=${encodeURIComponent(slotsArr[0])}` : ""}`;
...
exports.flushDayNoteReminders = onSchedule(
  ...
  async () => {
    ...
    for (const [key, note] of Object.entries(notes)) {
      ...
      await pushAdmin(title, body, { url: `https://admin.olhadrive.kiev.ua/?date=${dateStr}`, alarm: "1" });
